...
2. **c.388 & c.650 columns are built and verified.** Both marquee bills now have full 187-legislator columns in the tracker's **"By Legislator"** view (in `html`, generated by `build_voting_record.py` from the official roll calls in [`sources/rollcalls/`](./sources/rollcalls/)) and reconciled exactly to the printed tallies (74–73 / 19–15 and 76–71 / 18–16). Districts are populated from the rosters in [`sources/rosters/`](./sources/rosters/): Senate 35/35 and House 151/152. The one gap — Harriman of Lewiston, a special-election winner not yet on the district roster — is left blank, never guessed. Remaining older-bill tallies stay "TBD" until their roll calls are supplied.
